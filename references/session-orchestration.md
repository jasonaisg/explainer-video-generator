...
新初始化项目使用 1.2 版控制协议。旧项目已经正式验收的历史阶段不倒追撤销，但必须标为旧协议记录；旧项目尚未提交的阶段以及所有新提交，均立即采用 `SUBMISSION_AUTHORIZED` 规则。旧项目首次发生变更时运行 `change_control.py bootstrap`，补建产物依赖图与变更控制目录。

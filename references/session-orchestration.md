...
新初始化项目使用 1.1 版控制协议。旧项目已经正式验收的历史阶段不倒追撤销，但必须标为旧协议记录；旧项目尚未提交的阶段以及所有新提交，均立即采用 `SUBMISSION_AUTHORIZED` 规则。
...
2. 项目经理不得代替用户批准内容；项目经理只执行技术、完整性、证据和跨阶段验收。
...
5. 项目经理验收上述七个阶段前，必须直接读取 `approval-record.md`，确认最新事件是有效的 `SUBMISSION_AUTHORIZED`，并核对最终产物、版本、范围、用户授权原话、零未决事项、已关闭条件、已完成问答及时间。不得接受“用户应该同意了”等二手描述。
...
审阅过程中的批注、提问、修改要求或内容认可，使用 `record-review` 记录。此命令会撤销此前尚未提交的授权：

```text
conda run -n conda_video python scripts/projectctl.py record-review <项目目录> Pxx --event QUESTION --artifact <产物路径或编号> --version <版本> --message <用户问题或反馈>
```

`record-approval` 仅记录内容意见（`APPROVED`、`APPROVED_WITH_CONDITIONS` 或 `REVISION_REQUIRED`），同样不能授权提交。保留该命令是为了区分内容决定与最终提交授权。

所有事项关闭并冻结最终版本后，阶段 Session 必须先展示最终汇总，再单独询问用户是否授权提交项目经理。用户明确授权后使用：

单独询问时使用清楚而不诱导的表述，例如：`当前最终版本为 vX，批注、附加要求和问答均已处理，未决事项为 0。你是否明确授权我现在把这个版本提交给项目经理验收？` 在用户回答前不得发送项目经理通知。

```text
conda run -n conda_video python scripts/projectctl.py authorize-submit <项目目录> Pxx --artifact <最终产物路径或编号> --version <最终版本> --scope <提交范围> --authorization-quote <用户明确授权原话> --open-items 0 --conditions-status CLOSED --qa-status COMPLETE
```

只有 `authorize-submit` 能生成 `SUBMISSION_AUTHORIZED`。此后任何 `record-review`、`record-approval`、文件修改或新版本都会使授权失效；必须重新汇总并再次取得授权。提交与验收均以最新一个互动事件为准。

交付清单不得包含授权后仍会写入的 `approval-record.md` 或 `stage-result.json`；这两个是控制记录，不是冻结内容产物。提交授权会绑定交付清单及其所列产物的 SHA-256，授权后产物发生变化时提交器必须拒绝提交。
...
将变更归类为配置、源媒体、转录或时间轴、事实、设计、动效、字幕、素材或许可、混音、交付。找出最早受影响阶段，将其及所有依赖的已验收阶段标为 `STALE`。保留旧文件和旧审批，建立新版本或新尝试。没有替代记录时，不得覆盖已审批产物。

常见路由：

- 源 MP4 或 MP3 变化：返回 P01，并使全部下游失效。
- 文稿或转录时间变化：返回 P02，并使下游失效。
- 事实修正：返回 P03，并使受影响下游失效。
- 人物布局或动画选择变化：返回 P04，并使下游失效。
- 视觉识别或动效语言变化：返回 P05，并使下游失效。
- 仅字幕样式变化：使 P05、P08–P14 中依赖字幕的部分失效。
- 仅背景音乐或音效变化：使 P05、P08–P14 中依赖声音增强的部分失效。

...
当前版本：`1.5.0`。以仓库根目录的 `VERSION` 文件为版本权威来源。
...
- P04 必须先从完整口播形成有边界证据的认知切片，再为每个切片完整生成 V1、V2、V3 三套可执行方案，完成横向比较后才推荐；用户审批的是完整方案，不是抽象类别或内部代码。
- P05 必须为每个连续 A 编号建立独立动效脚本，详细描述动效类别、信息结构、视觉风格、元素动画、真实时间和转场，并为每个场景生成入场建立后、信息最完整时、交接前三张真实审阅图供用户逐场景批准。不得用汇总性的 `director-motion-script.md` 代替独立脚本或逐场景审阅。
...
5. 更新阶段结果、静态交付清单、交接、问题和审批记录；`deliverables-manifest.json` 只保存冻结产物事实与哈希，不得保存授权、提交、等待或推进状态，也不得包含可变治理控制记录。
...
动态流程状态只以 `00_control/project-state.json` 和 `approval-record.md` 最新互动事件为权威。提交前运行 `projectctl.py validate-control-consistency <项目目录>`；PM 退回后必须重新冻结并重新取得提交授权。内容修订可以改变内容产物；控制修订必须记录并复核受保护内容修改前后的 SHA-256，不得借机重评或改变用户内容决定。
